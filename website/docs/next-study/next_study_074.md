...
```mermaid
flowchart TD
  R["app/"] --> G["(site)/  ※URLに出ない(整理用)"]
  G --> L["layout.tsx  (共通の外枠)"]
  G --> H["page.tsx  → /"]
  G --> A["about/page.tsx  → /about"]

  G --> P["posts/  → /posts ..."]
  P --> P0["page.tsx  → /posts"]
  P --> PL["loading.tsx  (posts区間だけ⏳)"]
  P --> PE["error.tsx  (posts区間だけ🧯)"]
  P --> PID["[id]/"]
  PID --> P1["page.tsx  → /posts/:id"]
  PID --> PN["not-found.tsx  (posts/:id の404🚪)"]

  G --> D["dashboard/  (Parallel Routes例)"]
  D --> M["@main/"]
  D --> S["@side/"]
  M --> MP["page.tsx  (main枠)"]
  S --> SP["page.tsx  (side枠)"]
  ...
  P --> MOD["@modal/  (モーダル枠)"]
  MOD --> INT["(.)[id]/page.tsx  (Intercepting例🪄)"]
```
