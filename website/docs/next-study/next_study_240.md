...
```mermaid
flowchart TD
  L[app/layout.tsx\n共通metadata] --> M[Next.jsが統合して<head>生成]
  P[page.tsx\nmetadata / generateMetadata] --> M
  F[特殊ファイル\n(opengraph-image / icon / robots / sitemap など)] -->|優先度高め| M
  M --> H[HTMLの<head>タグ / ルート(robots.txt, sitemap.xml等)]
```
